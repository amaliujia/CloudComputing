...
for j in range(0, params['num_iter']):
    for i in range(0, blocks):
        key_data_filter = key_data.filter(lambda x : filter_blocks(x, i, blocks_col_dim, blocks))
        merged_rdd = key_data_filter.groupWith(W_rdd, H_rdd).partitionBy(num_workers)
